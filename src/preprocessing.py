...
def new_edge_index(data):
    # Original hyperedge index as torch tensors
    node_indices = data.edge_index[0]
    hyperedge_indices = data.edge_index[1]

    # Create a dictionary to map hyperedge indices to node lists
    hyperedge_dict = {}
    for node, edge in zip(node_indices.tolist(), hyperedge_indices.tolist()):
        if edge not in hyperedge_dict:
            hyperedge_dict[edge] = []
        hyperedge_dict[edge].append(node)

    # Generate pairs and new hyperedge index
    new_node_indices = []
    new_hyperedge_indices = []
    next_hyperedge_id = 0

    for nodes in hyperedge_dict.values():
        # Create all combinations of pairs for the nodes in the current hyperedge
        pairs = combinations(nodes, 2)
        for pair in pairs:
            new_node_indices.extend(pair)
            new_hyperedge_indices.extend([next_hyperedge_id, next_hyperedge_id])
            next_hyperedge_id += 1

    # Convert lists to torch tensors
    new_node_indices_tensor = torch.tensor(new_node_indices)
    new_hyperedge_indices_tensor = torch.tensor(new_hyperedge_indices)

    # Combine new_node_indices_tensor and new_hyperedge_indices_tensor into a single edge_index tensor
    new_edge_index = torch.stack([new_node_indices_tensor, new_hyperedge_indices_tensor], dim=0)
