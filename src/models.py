...
class SheafHyperGNN(nn.Module):
    ...
    def __init__(self, args, sheaf_type):
        ...
        self.use_fixed_alpha = args.ablation_fixed_sheaf
    ...
    def forward(self, data):
        ...
        if getattr(self.args, 'no_diffusion', False):
            # ✅ No diffusion: skip all conv layers
            x = x.view(num_nodes, -1)  # reshape back to node-level
            x = self.lin2(x)
            return x
        ...
        for i, conv in enumerate(self.convs[:-1]):
            ...
            if i == 0 or self.dynamic_sheaf:
                if self.use_fixed_alpha:
                    h_sheaf_index = edge_index  # shape: [2, E]
                    h_sheaf_attributes = torch.ones(edge_index.size(1), device=x.device)  # shape: [E]와 정확히 일치
                else:
